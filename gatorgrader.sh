...
if [ "$CHECK" = true ]; then
  ...
  # --> GatorGrader CHECK: at least 11 "println(" fragment exists in the code
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/practicalfive/experiment \
                                     --checkfiles Experiment.java --fragments "println(" --fragmentcounts 11
  determine_exit_code $?
  ...
  determine_exit_code $?
  # --> GatorGrader CHECK: the repository contains at least three commits (+3
  # for the current number in the starter repository)
